...
#+BEGIN_SRC emacs-lisp
  ...
  (defun kb/org-dir-show-headings (dir) ; An issue with this is that you open the file
    ...
    (let ((dir-initial default-directory)))
    (setq counsel-find-file-ignore-regexp "\\.el$")
    (cd dir)
    (counsel-find-file)
    (counsel-org-goto)
    (setq counsel-find-file-ignore-regexp regexp-initial)
    (cd dir-initial)
    (org-overview)
    (org-reveal)
    (org-show-subtree)
...
#+BEGIN_SRC emacs-lisp
  (kb/leader-keys
    ...
    "fcE" '((lambda () (interactive) (find-file kb/emacs-base-config-file)) :which-key "Open Emacs base config")
