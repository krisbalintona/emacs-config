...
This function was inspired by `pulsar--post-command-pulse'."
  ;; TODO 2024-10-24: Consider the case when there are multiple entries in
  ;; `krisb-reveal-fold-commands' that have the same :command.  In these cases,
  ;; the differentiator would be predicate, so we should try all of them until
  ;; there is a successful invocation.
  (when (cl-some (lambda (plist)
                   (when (plist-get plist :command)
                     (eq (plist-get plist :command) this-command)))
                 krisb-reveal-fold-commands)
    (let* ((plist (cl-find this-command krisb-reveal-fold-commands
                           :key (lambda (plist) (plist-get plist :command))
                           :test #'eq))
           (location (plist-get plist :location))
           (point (car location))
           (buffer (cdr location))
